	...
	void iniciar() { // Esse método faz, basicamente, o que a main original fazia (menu, seleções, entrada de texto, etc.)
		...
		while (op != 5) {
			...
			if (op == 4) {
				...
				if (op2 == 4) { // Totalização do número de obras por ano - Samuel vai implementar // sam
					cout << "Digite o ano " << endl;
					int ano;
					cin >> ano;
					quantidade = porAno(obras, ano);
					cout << "Quantidades de obras no ano " << ano << ": " << quantidade << endl;
				}

				if (op2 == 5) { // Totalização do número de obras por autor - Samuel vai implementar // sam
					cout << "Digite o nome do autor " << endl;
					string autor;
					getline(cin, autor);
					quantidade = porAutor(obras, autor);
					cout << "Quantidade de obras publicadas do autor " << autor << ": " << quantidade << endl;
				}
			}
			cout << endl;
		}
